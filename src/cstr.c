...
bool cstr_ends_with(
    const char *s, const size_t s_len, const char *pattern, const size_t pattern_len) {
	$assert_nonnull(s);
...
long long cstr_to_llong(const char *s, const long long _default) {
	$assert_nonnull(s != NULL);
	char *endptr;
...
unsigned long long cstr_to_ullong(const char *s, const unsigned long long _default) {
	$assert_nonnull(s != NULL);
	char *endptr;
