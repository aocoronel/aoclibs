...
void lex_printf(Lexer *l,
				const Source_Code_Location *loc,
				const char *color,
				const char *level,
				const char *fmt,
				...) $attr_printf(5, 6);
...
char lexer_expect(Lexer *l, char expected) {
	Source_Code_Location loc = l->loc;
...
Lexer_Token lexer_number(Lexer *l) {
	...
	if (base == 10 && (lexer_peek(l) == 'e' || lexer_peek(l) == 'E')) {
		...
		Source_Code_Location save_loc = l->loc;
...
Lexer lexer_init(const char *filename, const char *contents, size_t length, Lexer_Config settings) {
	Source_Code_Location loc = { .filename = filename, .line = 0, .column = 0 };
