...
`debug/heap.h` contains a couple of macros that replace `malloc`, `realloc`, `calloc` and `free` and store all operations.

Currently, this has a `debug_memory_summary`, which writes a summary of how many allocations and frees have been made across the program execution. It also shows which functions allocated in the heap, in which file and line, that have not been freed.
